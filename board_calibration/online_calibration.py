...
if use_charuco:
	print('Visual calibration plate type: ChArUco Boards')
else:
	print('Visual calibration plate type: Chess Boards')
...
rotation_vectors = FixedSizeArray(max_len=15, data_size=(0, 3))
translation_vectors = FixedSizeArray(max_len=15, data_size=(0, 3))
chessboard_positions_t = FixedSizeArray(max_len=15, data_size=(0, 3))
...
while cap.isOpened():
	...
	if start_calibration and time.time() - start_time >= capture_interval:
		...
		if use_charuco:
			...
			corners_img, ids, rejectedImgPoints = cv2.aruco.detectMarkers(img, aruco_dict)
			if len(corners_img) > 0:
				found, charucoCorners, charucoIds = cv2.aruco.interpolateCornersCharuco(corners_img, ids, img,
																						board)
				if found:
			...
			found, corners_img = cv2.findChessboardCorners(img, pattern_size)
			if found:
				retval, rvec, tvec = cv2.solvePnP(object_points, corners_img, camera_matrix, dist_coeffs)
